#lang racket
(require 2htdp/universe racket/draw racket/generator images/logos racket/flonum)
;----------- const --------------------
(define log2 (log 2))
...
(define bailout (make-parameter 100.0))
(define center (make-parameter 0))
;-------------------- others --------------------------
(define radius (make-derived-parameter bailout sqr sqrt))
(define view (make-parameter 600+450i))
(define scale (make-parameter 225.0))
(define (set-size! x)
  (scale (* 1/2 (min (real-part x) (imag-part x))))
  (view x))

(define sac-stripe (make-parameter 5.0))
(define sac-trunc (make-parameter 1))
;----------------------- fractal types ---------------------------
(define (mandelbrot-gen c)
  (let ([i (fl/ 2.0 (scale))])
    (generator ()
               (let loop ([z c] [x2 (fl/ (flexpt (flreal-part c) 2.0) (scale))]
                          [y2 (fl/ (flexpt (flimag-part c) 2.0) (scale))])
               (yield z x2 y2)
               (let ([t (make-flrectangular (fl+ (flreal-part c) (fl- x2 y2))
                                            (fl+ (flimag-part c) (fl* i (flreal-part z) (flimag-part z))))])
                 (loop t (fl/ (flexpt (flreal-part t) 2.0) (scale))
                       (fl/ (flexpt (flimag-part t) 2.0) (scale))))))))
(define (bship-gen c)
  (let ([i (fl/ 2.0 (scale))])
    (generator ()
               (let loop ([z c] [x2 (fl/ (flexpt (flreal-part c) 2.0) (scale))]
               ...
               (yield z x2 y2)
               (let ([t (make-flrectangular (fl+ (flreal-part c) (fl- x2 y2))
                                            (fl- (flimag-part c) (flabs (fl* i (flreal-part z) (flimag-part z)))))])
                 (loop t (fl/ (flexpt (flreal-part t) 2.0) (scale))
                       (fl/ (flexpt (flimag-part t) 2.0) (scale))))))))
(define (julia-gen p)
  (let ([j (exact->inexact (* p (scale)))] [i (fl/ 2.0 (scale))])
  (λ (c) (generator ()
                    (let loop ([z c] [x2 (fl/ (flexpt (flreal-part c) 2.0) (scale))]
                          [y2 (fl/ (flexpt (flimag-part c) 2.0) (scale))])
               (yield z x2 y2)
               (let ([t (make-flrectangular (fl+ (flreal-part j) (fl- x2 y2))
                                            (fl+ (flimag-part j) (fl* i (flreal-part z) (flimag-part z))))])
                 (loop t (fl/ (flexpt (flreal-part t) 2.0) (scale))
                       (fl/ (flexpt (flimag-part t) 2.0) (scale)))))))))
(define (bship-test c)
  (or (and (zero? (flimag-part c))
       (fl<= (flreal-part c) (fl/ (scale) 4.0))
       (fl>= (flreal-part c) (fl* -2.0 (scale))))
      (and (positive? (flimag-part c))
           (fl<= (flreal-part c) (fl/ (scale) 4.0))
           (let ([ys (flexpt (flimag-part c) 2.0)])
             (or (fl< (fl/ (fl+ (flexpt (fl+ (flreal-part c) (scale)) 2.0) ys) (scale)) (fl/ (scale) 16.0))
                 (let* ([t (fl- (flreal-part c) (fl/ (scale) 4.0))]
                        [q (fl/ (fl+ (flexpt t 2.0) ys) (scale))])
                   (fl< (fl* q (fl+ q t)) (fl/ ys 4.0))))))))
...
(define (potential gen test start)
  (let ([ls (log (scale))] [b (fl* (scale) (bailout))])
    (λ (c) (if (test c) 0.0
               ((λ (p) (if (fl<= (cdr p) b) 0.0
                           (flexpt (fl* 4.0 (fl/ (fl- (fllog (cdr p)) ls)
                                                 (->fl (arithmetic-shift 1 (add1 (car p))))))
                                 (fl/ 1.0 (->fl (+ 7 (integer-length (exact-floor (zoom)))))))))
                (point-loop (n start) (gen z x2 y2) #f c (cons n (fl+ x2 y2))))))))
(define (smooth-par b b2 lb ls)
  (λ (mag) (cond [(fl<= mag b) 1.0]
                 [(fl>= mag b2) 0.0]
                 [else (fl+ 1.0 (fl- lb (fl/ (fllog (- (fllog mag) ls)) log2)))])))
(define (smooth gen test start)
  (let ([par (smooth-par (* (scale) (bailout)) (* (scale) (flexpt (bailout) 2.0))
                         (fl/ (fllog (fllog (bailout))) log2) (fllog (scale)))])
    (λ (c) (if (test c) 0
               ((λ (p) (fl- 1.0 (/ (fl+ (->fl (car p)) (par (cdr p))) (->fl (add1 (limit))))))
                (point-loop (n start) (gen z x2 y2) #f c (cons n (+ x2 y2))))))))
(define (dem-out ls b)
  (λ (d mag)
    (if (or (zero? d) (fl<= mag b)) 0
        (flexpt (fl* 4.0 (fl- (log mag) ls)
                 (sqrt (fl/ mag (fl* (scale) (fl+ (flexpt (real-part d) 2.0) (flexpt (imag-part d) 2.0))))))
              (fl/ 1.0 (->fl (+ 3 (integer-length (exact-floor (zoom))))))))))
(define (dem gen test start [init-d 1] [add 1])
  (let ([i (fl/ 2.0 (scale))] [out (dem-out (log (scale)) (fl* (scale) (bailout)))])
...
(define (sac-t z)
  (fl* 0.5 (fl+ 1.0 (sin (fl* (sac-stripe) (angle z))))))
(define ((sac-out par) a-1 n z mag)
  (if (or (>= n (limit)) (< n (sac-trunc))) 0
      (let ([d (par mag)])
        (fl+ (fl* d (fl/ (fl+ a-1 (sac-t z)) (->fl (- n -1 (sac-trunc)))))
           (if (> n (sac-trunc)) (fl* (fl- 1.0 d) (fl/ a-1 (->fl (- n (sac-trunc))))) 0.0)))))
(define (sac gen test start)
  (let* ([b (fl* (scale) (bailout))]
         [out (sac-out (smooth-par b (fl* (scale) (flexpt (bailout) 2.0))
                                   (fl/ (fllog (fllog (bailout))) log2) (fllog (scale))))])
    (λ (c) (let ([a-1 0.0])
             (if (test c) 0.0
                 ((λ (lst) (apply out a-1 lst))
                  (point-loop (n start) (gen z x2 y2) #f c
                              (let ([mag (fl+ x2 y2)])
                                (unless (or (< n (sac-trunc)) (fl> mag b))
  ...
  (syntax-case stx ()
    [(_ (n start) (gen z x2 y2) break c body ...)
     #'(for/last ([n (in-range start (add1 (limit)))]
                  ...
                  #:final (fl> (fl/ (fl+ x2 y2) (scale)) (bailout)))
         #:break break
         body ...)]))
         ...
         (parameterize ([scale (* (zoom) (scale))])
           (let ([call esc])
             (for ([y (center-range (imag-part t) (imag-part c) -1)]
                   ...
                   [a (in-naturals)])
               (let ([p (make-rectangular a b)] [val (call (make-flrectangular (->fl x) (->fl y)))])
                 body ...)))))]))
...
(define text-color (make-parameter (make-object color% 0 0 0)))
...
(define-syntax-rule (fractal-bmp-thread bmp)
  (thread (λ () (fractal-loop pos z (v (case (type)
                              ...
                              (send bmp set-argb-pixels (real-part pos) (imag-part pos) 1 1 ((coloring) v))))))
...
(big-bang (refresh (make-object bitmap% (real-part (view)) (imag-part (view))))
  ...
  [on-key (λ (w k) (cond [(key=? k "\t")
                          (text-color (make-object color% (- 255 (send (text-color) red))
                                        (- 255 (send (text-color) green))
                                        (- 255 (send (text-color) blue)))) w]
                         [(key=? k "\r") (new-world w #t)]
                         [(key=? k "left") (center (- (center) (/ 1/4 (zoom)))) (new-world w)]
                         [(key=? k "right") (center (+ (center) (/ 1/4 (zoom)))) (new-world w)]
                         [(key=? k "up") (center (+ (center) (/ +1/4i (zoom)))) (new-world w)]
                         [(key=? k "down") (center (- (center) (/ +1/4i (zoom)))) (new-world w)]
                         [(key=? k "add") (zoom (* 2 (zoom))) (new-world w)]
